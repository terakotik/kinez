...
export default function App() {
  ...
  return (
    <div className="h-screen bg-slate-50 text-slate-800 font-sans selection:bg-indigo-100 selection:text-indigo-900 overflow-hidden flex flex-col">
      
      {/* Header */}
      <header className="h-16 px-10 flex items-center justify-between bg-white border-b border-slate-200 shrink-0 z-50">
      ...
      <main className="flex-1 p-6 flex flex-col gap-6 overflow-hidden min-h-0">
        ...
        <AnimatePresence mode="wait">
          {step < 3 ? (
            ...
            >
              {/* 1. КАТАЛОГ */}
              <div className="flex-1 flex flex-col bg-white border border-slate-200 rounded-[32px] shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-100">
                  ...
                  <div className="relative">
                    <input 
                      ...
                      placeholder="Поиск..." 
                      className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none pl-10 focus:ring-2 focus:ring-indigo-500/20"
                    />
              ...
              <div className="flex-1 flex flex-col bg-slate-100/50 border border-slate-200 rounded-[32px] shadow-sm overflow-hidden">
                      ...
                      allIdentifiedRemediesArr.map(remKey => {
                        ...
                        return (
                          <motion.div 
                            ...
                            className={`p-5 rounded-2xl bg-white border-t-4 shadow-sm border border-slate-100 group transition-shadow hover:shadow-md`} 
                          ...
                          >
                            ...
                            <div className="space-y-2">
                              {etItems && <div className="flex flex-wrap gap-1"><span className="w-full text-[7px] text-indigo-300 uppercase font-black">Этиология:</span>{etItems.map(s => <span key={s} className="text-[8px] bg-slate-50 px-2 py-0.5 rounded-full text-slate-500 font-bold border border-slate-100">{s}</span>)}</div>}
                              {noItems && <div className="flex flex-wrap gap-1"><span className="w-full text-[7px] text-amber-400 uppercase font-black">Нозология:</span>{noItems.map(s => <span key={s} className="text-[8px] bg-amber-50 px-2 py-0.5 rounded-full text-amber-600 font-bold border border-amber-100">{s}</span>)}</div>}
                            </div>
                          </motion.div>
                        );
                      })
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
                  <AnimatePresence mode="popLayout" initial={false}>
                    {targetOrgansArr.length === 0 ? (
                      <motion.div key="empty-organs" initial={{ opacity: 0 }} animate={{ opacity: 0.2 }} className="h-full flex flex-col items-center justify-center"><Activity className="w-10 h-10 mb-2" /><span className="text-[9px] font-black uppercase">Ожидание выбора</span></motion.div>
                    ) : (
            ...
            <motion.div 
              ...
              className="flex-1 flex flex-col md:flex-row gap-8 overflow-hidden bg-white rounded-[40px] border border-slate-200 shadow-2xl p-4 md:p-8"
            >
              <div className="flex-1 flex flex-col min-h-0">
                <div className="flex items-center mb-8 border-b border-slate-100 pb-6 shrink-0">
                   ...
                   <div>
                      <h2 className="text-2xl font-black tracking-tight text-slate-900 leading-none mb-1 text-slate-800 uppercase">Итоговый анамнез</h2>
                   ...
                   <section>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                         <div className="p-5 bg-slate-50 rounded-3xl border border-slate-100">
                            <h4 className="text-[9px] font-black text-slate-400 uppercase mb-3">Этиологические факторы:</h4>
                            <div className="flex flex-wrap gap-2">
                               {Array.from(selectedSymptoms).map(s => <span key={s} className="px-3 py-1.5 bg-white border border-slate-200 rounded-xl text-[10px] font-bold text-slate-700 shadow-sm">{s}</span>)}
                            </div>
                         </div>
                         <div className="p-5 bg-indigo-50/30 rounded-3xl border border-indigo-100">
                            <h4 className="text-[9px] font-black text-indigo-400 uppercase mb-3">Нозологические единицы:</h4>
                            <div className="flex flex-wrap gap-2">
                               {Array.from(selectedNosology).map(s => <span key={s} className="px-3 py-1.5 bg-white border border-indigo-200 rounded-xl text-[10px] font-bold text-indigo-700 shadow-sm">{s}</span>)}
                            </div>
                         </div>
                      </div>
                   </section>
              ...
              <div className="w-full md:w-80 flex flex-col shrink-0 gap-6 min-h-0">
                 ...
                 <div className="mt-auto space-y-3">
                    ...
                    <button onClick={() => setStep(2)} className="w-full text-center py-2 text-[9px] font-bold text-slate-400 uppercase hover:text-indigo-600 transition-colors">Назад к нозологии</button>
                 </div>
              </div>
            </motion.div>
          )}
